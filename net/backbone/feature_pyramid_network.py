...
class FeaturePyramidNetwork(nn.Module):
    ...
    def __init__(self, in_channels_list, out_channels, extra_blocks=None, use_eca=0):
        ...
        self.inner2_blocks = nn.ModuleList()  # 原名为inner_blocks，不过因为旧的参数会载入，所以换了
        # 对调整后的特征矩阵使用3x3的卷积核来得到对应的预测特征矩阵
        self.layer_blocks = nn.ModuleList()
        for in_channels in in_channels_list:
            if in_channels == 0:
                continue
            inner_block_module = nn.Conv2d(in_channels, out_channels, 3, padding=1)
            print('inner_block_module，原本用于调整通道到256，经过实验发现11卷积会损失信息，故改用33')
            layer_block_module = nn.Conv2d(out_channels, out_channels, 3, padding=1)
            self.inner2_blocks.append(inner_block_module)
    ...
    def get_result_from_inner_blocks(self, x, idx):
        ...
        """
        num_blocks = len(self.inner2_blocks)
        if idx < 0:
            idx += num_blocks
        i = 0
        out = x
        for module in self.inner2_blocks:
            if i == idx:
                out = module(x)
            i += 1
        return out
