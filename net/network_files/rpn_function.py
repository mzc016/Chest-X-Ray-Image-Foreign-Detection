...
class AnchorsGenerator(nn.Module):
    ...
    def generate_anchors(self, scales, aspect_ratios, dtype=torch.float32, device=torch.device("cpu")):
        ...
        Arguments:
            ...
            device: cpu/gpu
        """
        # scales就是anchor基础大小，只包含一个数字，也就是尺度为1的tensor
        # 每个scales对应三个比例尺
...
def permute_and_flatten(layer, N, A, C, H, W):
    ...
    # 调换tensor维度
    layer = layer.permute(0, 3, 4, 1, 2)  # [N, H, W, -1, C]  4,200,200,3,1
    layer = layer.reshape(N, -1, C)  # 4,12w, 1
    return layer
...
def concat_box_prediction_layers(box_cls, box_regression):
    ...
    """
    #  原来是两个列表，每个列表元素各自为 4*3*200*200   4*12*200*200  4张图片，200*200的区域，每个point产生3个/12个特征
    box_cls_flattened = []
    box_regression_flattened = []

    # 遍历每个预测特征层
    for box_cls_per_level, box_regression_per_level in zip(box_cls, box_regression):  # 遍历tensor
        # [batch_size, anchors_num_per_position * classes_num, height, width]
        # 注意，当计算RPN中的proposal时，classes_num=1,只区分目标和背景
        N, AxC, H, W = box_cls_per_level.shape  # 4, 3, 200, 200
        # # [batch_size, anchors_num_per_position * 4, height, width]
        Ax4 = box_regression_per_level.shape[1]  # 12
        ...
        # [N, -1, C]
        # # 4,3, 1,200,200 ->  4, 120000, 1
        box_cls_per_level = permute_and_flatten(box_cls_per_level, N, A, C, H, W)
        box_cls_flattened.append(box_cls_per_level)

        # [N, -1, C]  4, 3, 4, 200, 200 -> 4, 120000, 4
        box_regression_per_level = permute_and_flatten(box_regression_per_level, N, A, 4, H, W)
        box_regression_flattened.append(box_regression_per_level)

    # [4*120000*1, 4*30000*1, 4*7500*1, 4*1875*1, 4*507,1] -> 4*159882 *1  4张图片，汇聚所有fpn上的anchor，共159882个，1个类
    # 4*159882*1  -> 639528*1，同一个batch的所有anchor进行了综合，也就是flatten的操作
    box_cls = torch.cat(box_cls_flattened, dim=1)
    box_cls = box_cls.flatten(0, -2)  # start_dim, end_dim
    # 同理，列表内tensor按照dim1进行对齐  得到 4*159882*4的tensor ， 4张图， 每张图159882个anchor，每个anchor对应4个坐标
    # 这里用reshape代替flatten，结果也是同理的 所有的anchor  639528*4，每个anchor4个坐标偏移
...
class RegionProposalNetwork(torch.nn.Module):
    ...
    # 我自己加了个anchor，用来计算iou损失
    # 这里关于坐标的tensor：pred_bbox_deltas为639528*4 ,regression_targets是长度为4的tuple每个元素为159882*4
    # 而anchors是长度为4的列表，每个都是159882*4
    # 4列的含义依次为dx dy dw dh    而 anchor的含义是 x1 y1 x2 y2
    # def compute_loss(self, objectness, pred_bbox_deltas, labels, regression_targets):
    def compute_loss(self, objectness, pred_bbox_deltas, labels, anchors, matched_gt_boxes):
        # type: (Tensor, Tensor, List[Tensor], List[Tensor], List[Tensor]) -> Tuple[Tensor, Tensor]
        """
        ...
        Arguments:
            ...
            anchors：anchors的原坐标
            matched_gt_boxes：anchor对应的gt框，左上角和右下角坐标
        ...
        # regression_targets = torch.cat(regression_targets, dim=0)  # xywh的回归值
        anchors = torch.cat(anchors, dim=0)  # 直接全部anchor聚合，四个坐标x1y1x2y2
        matched_gt_boxes = torch.cat(matched_gt_boxes, dim=0)  # 直接聚合匹配的gtbox，四个坐标也是x1y1x2y2

        # 计算边界框回归损失
        # box_loss = det_utils.smooth_l1_loss(  # 这里只取了正样本
        #     pred_bbox_deltas[sampled_pos_inds],
        #     regression_targets[sampled_pos_inds],
        #     beta=1 / 9,
        #     size_average=False,
        # ) / (sampled_inds.numel())
        box_loss = det_utils.c_IoU_loss(
            pred_bbox_deltas[sampled_pos_inds],
    ...
    def forward(self,
        ...
        # objectness和pred_bbox_deltas都是list，list包含4个元素，每个元素对应一个fpn层，是4*3*200*200和4*12*200*200tensor
        # 200*200 -> 100*100 -> 50*50 依次变小
        # 说明后两个尺寸是size，第一个尺寸是batch，第二个尺寸是每个位置的anchor数量对应的类别置信度和回归值
        objectness, pred_bbox_deltas = self.head(features)

        # 生成一个batch图像的所有anchors信息,list(tensor)元素个数等于batch_size
        # 这里的anchor也是列表，4个图像属于这个batch的意思，每一个图像包含了打包好的tensor，包括这个图像对应的所有fpn层产生的所有anchor
        # 每个列表元素，也就是每个图像中的anchor包的tensor显然是 159882*4
        anchors = self.anchor_generator(images, features)

        # batch_size
        num_images = len(anchors)  # 4张图象

        # numel() Returns the total number of elements in the input tensor.
        # 计算每个预测特征层上的对应的anchors数量
        num_anchors_per_level_shape_tensors = [o[0].shape for o in objectness]  # [3, 200, 200] 每个tensor的第一个维度内
        num_anchors_per_level = [s[0] * s[1] * s[2] for s in num_anchors_per_level_shape_tensors]  # 12w、3w等

        # 调整内部tensor格式以及shape  ， 最终变成了  639528*1 和639528*4的tensor 表示所有的anchor和他对应的类别以及坐标回归值
        ...
        if self.training:
            ...
            # list类型，每个元素为当前图像每个anchor的正负样本（1，0，-1，对应正样本，负样本和无用样本）
            # matched_gt_boxes也是列表，每个元素为当前图像种每个anchor对应的gtbox坐标，样本和丢弃样本默认匹配0号gtbox
            labels, matched_gt_boxes = self.assign_targets_to_anchors(anchors, targets)
            # 结合anchors以及对应的gt，计算regression参数
            regression_targets = self.box_coder.encode(matched_gt_boxes, anchors)
            #  这里关于坐标的tensor：pred_bbox_deltas是为639528*4、
            #  regression_targets为tuple， 也就是4个batch，每个元素159882*4(经过了split，成了列表)
            #  而anchors是长度为4的列表，每个都是159882*4
            #  4列的含义依次为dx dy dw dh    而 anchor的含义是 x1 y1 x2 y2
            ...
            # )
            loss_objectness, loss_rpn_box_reg = self.compute_loss(
                # 这里其实可以人为定义预测的四个坐标是啥东西，我认为它是四个坐标，那他就是四个坐标
                objectness, pred_bbox_deltas, labels, anchors, matched_gt_boxes
            )
            losses = {
